...
import gymScene from "./Gym1.glb";
import Locker from "../assets/locker";
// import {css, github, html, javascript, react, 
//   tailwindcss, typescript, linkedin,
...
const Gym = ({ onCameraChange, currentView, onPopupTrigger }) => {
  const gymRef = useRef();
  const { nodes, materials, animations  } = useGLTF(gymScene);
  const { actions } = useAnimations(animations, gymRef);
  ...
  useEffect(() => {
    ...
    // Check if we have the character3 animation
    if (actions["character3"]) {
      console.log("Starting character3 animation");
      actions["character3"].reset().play();
      actions["character3"].setLoop(THREE.LoopRepeat);
      actions["character3"].setEffectiveTimeScale(1);
    } else {
      console.log("character3 animation not found");
      // If "character3" doesn't exist, try to play the first available animation
      const animationNames = Object.keys(actions);
      if (animationNames.length > 0) {
        const firstAnimationName = animationNames[1];
        console.log(`Playing first available animation: ${firstAnimationName}`);
        actions[firstAnimationName].reset().play();
        actions[firstAnimationName].setLoop(THREE.LoopRepeat);
        actions[firstAnimationName].setEffectiveTimeScale(1);
      } else {
